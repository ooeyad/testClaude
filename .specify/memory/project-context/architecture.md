...
functions. This is consistent across every file; match it (XI-1, XI-2).
...
| A ladder | `L.ladder(x, top, bottom)` in a level's `ladders` array | a solid with a flag — see INV-20 |
| Movement the shared machine cannot express | a subclass plus `ctor:` in the type definition, delegating everything else to `P.Enemy` | new states on `Enemy.prototype.update` |

### Ladder authoring rules
`top` and `bottom` are the y of the **surfaces where feet rest**, not decorative
extents — a climber is clamped to them on arrival. `top` is the smaller value
(y grows downward). Both ends must sit within `Climber.LEVEL_TOL` (24 px) of the
planes they claim to join, or the ladder is never selected (INV-21). Nothing
enforces this at runtime; a mis-authored ladder is silently invisible to the AI.
...
readable. The shared machine still has no jump, fall or climb state and no
pathfinding of any kind — movement between elevations lives in a **subclass**
instead. `Enemies.spawn` builds `cfg.ctor || P.Enemy`, so a type that needs
behaviour the shared machine cannot express names its own constructor; every type
that does not is built exactly as before. `P.Climber` is the only one today.
...
attacks call it, or `meleeSweep` / `explode` — never touch `hp` directly (XI-6).
